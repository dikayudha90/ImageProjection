...
/*bool VisualOdometryStereo::process(const vector<Matcher::p_match>& good_match){
	copy(good_match.begin(), good_match.end(), back_inserter(p_matched));
	for(int i = 0; i < p_matched.size(); ++i){
		cout << p_matched[i].u1c << " " << p_matched[i].u2c << " " << p_matched[i].u1p << " " << p_matched[i].u2p << endl;
	}
	return updateMotion();
}*/
